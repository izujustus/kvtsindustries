...
import { useActionState, useEffect, useState } from 'react'; // Kept your original import
import { createCustomer, createInvoice, recordPayment } from '@/app/lib/sales-actions';
...
// 2. INVOICE FORM (With Tax & Logistics)
export function InvoiceForm({ customers, products, onClose }: { customers: any[], products: any[], onClose: () => void }) {
  const [state, action, isPending] = useActionState(createInvoice, undefined);
  ...
  return (
    <form action={action} className="space-y-4">
            ...
            <div className="flex justify-between items-center text-sm">
               <div className="flex items-center gap-2">
                 ...
                 <input 
                    ...
                    className="w-16 h-7 text-right text-xs border border-gray-300 rounded"
                 />
               </div>
            ...
            <div className="flex justify-between items-center text-sm">
               <div className="flex items-center gap-2">
                 ...
                 <input 
                    ...
                    className="w-24 h-7 text-right text-xs border border-gray-300 rounded"
                 />
               </div>
      ...
      {state?.message && !state.success && (
         <p className="text-sm text-red-600 text-right bg-red-50 p-2 rounded">{state.message}</p>
      )}

      <div className="flex justify-end gap-2 pt-2">
        <button type="button" onClick={onClose} className="px-4 py-2 text-sm text-gray-600">Cancel</button>
        <button disabled={isPending || items.length === 0} className="bg-[#E30613] text-white px-4 py-2 rounded text-sm hover:bg-red-700 disabled:opacity-50">
          {isPending ? 'Generating...' : 'Generate Invoice'}
        </button>
      </div>
    </form>
