  ...
  return (
    <>
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-gray-200 pb-4">
        <div className="flex gap-4">
          <button 
            onClick={() => { setTab('INVOICES'); setSearchTerm(''); }} 
            className={clsx("text-sm font-medium pb-4 -mb-4 border-b-2 transition-colors", tab === 'INVOICES' ? "border-[#E30613] text-[#E30613]" : "border-transparent text-gray-500 hover:text-gray-700")}
          >
            Invoices & Waybills
          </button>
          <button 
            onClick={() => { setTab('CUSTOMERS'); setSearchTerm(''); }} 
            className={clsx("text-sm font-medium pb-4 -mb-4 border-b-2 transition-colors", tab === 'CUSTOMERS' ? "border-[#E30613] text-[#E30613]" : "border-transparent text-gray-500 hover:text-gray-700")}
          >
            Debtors List
          </button>
        </div>
                  ...
                  <td className="px-6 py-4 text-right space-x-2 flex justify-end items-center">
                    ...
                    {!inv.isPosted ? (
                      <>
                        <button 
                          onClick={() => handleEdit(inv)}
                          className="inline-flex items-center gap-1 text-gray-600 hover:text-blue-600 px-3 py-1.5 rounded text-xs font-bold transition-colors border border-gray-300 bg-white hover:bg-gray-50 mr-2"
                        >
                          <Pencil className="w-3 h-3" /> Edit
                        </button>
                        <button 
                          onClick={() => handleApprove(inv)}
                          className="inline-flex items-center gap-1 text-white bg-blue-600 hover:bg-blue-700 px-3 py-1.5 rounded text-xs font-bold transition-colors shadow-sm"
                        >
                          <FileCheck className="w-3 h-3" /> Approve & Post
                        </button>
                      </>
                    ) : (
                    /* POSTED ACTIONS */
                      <>
                        <button onClick={() => handleViewInvoice(inv)} className="text-gray-500 hover:text-black p-1" title="View Invoice"><Eye className="w-4 h-4" /></button>
                        {inv.waybill ? (
                          <button onClick={() => handleViewWaybill(inv)} className="text-gray-500 hover:text-[#E30613] p-1" title="View Waybill"><Truck className="w-4 h-4" /></button>
                        ) : (
                           <span className="text-xs text-red-400">No Waybill</span>
                        )}
                      </>
