...
import { useState } from 'react';
import clsx from 'clsx';
import { Plus, UserPlus, FileText, MoreHorizontal, Search, Filter, User, Clock, History, CreditCard, Edit } from 'lucide-react';
import { Modal } from '@/app/ui/users/user-form';
import { EmployeeForm, PayrollForm, EmployeeStatusForm } from '@/app/ui/hr/hr-forms';
...
function getTableTenure(hireDate: string | Date) {
  if (!hireDate) return { label: 'N/A', isLoyal: false };
  const start = new Date(hireDate);
  const now = new Date();
...
// --- MAIN COMPONENT ---
export default function HRClientWrapper({ employees = [], payrolls = [], departments = [], userRole }: any) {
  const [tab, setTab] = useState('EMPLOYEES');
  const [modal, setModal] = useState<'EMPLOYEE' | 'PAYROLL' | 'STATUS' | 'VIEW' | null>(null);
  const [selectedEmployee, setSelectedEmployee] = useState<any>(null);
  ...
  // Filter Payrolls for selected employee
  const employeePayrolls = selectedEmployee ? (selectedEmployee.payrolls || []) : [];
  ...
  return (
    <>
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-gray-200 pb-4">
        ...
        <div className="flex gap-2">
           <button onClick={() => setModal('PAYROLL')} className="flex items-center gap-2 px-3 py-2 bg-green-600 text-white rounded-md text-sm hover:bg-green-700 shadow-sm transition-all"><FileText className="w-4 h-4" /> Run Payroll</button>
           <button onClick={() => { setSelectedEmployee(null); setModal('EMPLOYEE'); }} className="flex items-center gap-2 px-3 py-2 bg-[#E30613] text-white rounded-md text-sm hover:bg-red-700 shadow-sm transition-all"><UserPlus className="w-4 h-4" /> Hire Employee</button>
        </div>
      </div>
        ...
        {tab === 'PAYROLL' && (
          <div className="overflow-auto flex-1 rounded-xl">
            <table className="min-w-full divide-y divide-gray-200 relative">
              ...
              <tbody className="divide-y divide-gray-200 bg-white">
                {payrolls.length === 0 ? (
                    <tr><td colSpan={3} className="px-6 py-12 text-center text-gray-400 text-sm">No payroll history found.</td></tr>
                ) : (
                    payrolls.map((p: any) => (
                    <tr key={p.id} className="hover:bg-gray-50">
                        <td className="px-6 py-4 text-sm text-gray-500 whitespace-nowrap">{new Date(p.paymentDate).toLocaleDateString()}</td>
      ...
      {/* --- MODALS --- */}
      {/* PASS DEPARTMENTS TO EMPLOYEE FORM */}
      <Modal isOpen={modal === 'EMPLOYEE'} onClose={() => setModal(null)} title="Hire New Employee"><EmployeeForm onClose={() => setModal(null)} departments={departments} /></Modal>
      <Modal isOpen={modal === 'PAYROLL'} onClose={() => setModal(null)} title="Process Payroll"><PayrollForm employees={employees} onClose={() => setModal(null)} /></Modal>
      <Modal isOpen={modal === 'STATUS'} onClose={() => setModal(null)} title="Manage Employee Status">{selectedEmployee && <EmployeeStatusForm employee={selectedEmployee} onClose={() => setModal(null)} />}</Modal>

      {/* PROFILE MODAL: VIEW / EDIT MODE */}
      <Modal isOpen={modal === 'VIEW'} onClose={() => setModal(null)} title={isEditing ? "Edit Employee Profile" : "Employee Profile"}>
        {selectedEmployee && (
          isEditing ? (
            // EDIT MODE
            <EmployeeForm 
              ...
              onClose={() => { setIsEditing(false); setModal(null); }} 
            />
          ) : (
            // VIEW MODE (Read Only - Rich Profile)
            <div className="space-y-6">
              {/* Header */}
              <div className="flex justify-between items-start bg-gray-50 p-4 rounded-xl border border-gray-100">
                <div className="flex gap-4">
                  <div className="w-16 h-16 rounded-full bg-white border border-gray-200 flex items-center justify-center text-2xl font-bold text-gray-500 shadow-sm overflow-hidden">
                    {selectedEmployee.imageUrl ? (
                        <img src={selectedEmployee.imageUrl} alt="Profile" className="w-full h-full object-cover" />
                    ) : (
                ...
                <div className="flex flex-col items-end gap-2">
                   {isAdmin && (
                     <button onClick={handleEditClick} className="flex items-center gap-1 text-xs font-bold bg-[#E30613] text-white px-3 py-1.5 rounded hover:bg-red-700 transition-colors">
                       <Edit className="w-3 h-3"/> Edit Profile
                     </button>
                   )}
                   <button onClick={() => setModal('STATUS')} className="text-xs font-bold text-gray-500 hover:text-gray-900">Change Status</button>
                </div>
              </div>

              {/* TABS FOR VIEW (Personal, Bank, Job) */}
              <div className="grid grid-cols-2 gap-6 text-sm">
                 <div className="bg-gray-50 p-4 rounded border border-gray-100">
                    <h3 className="font-bold text-gray-900 mb-3 flex items-center gap-2"><User className="w-3 h-3"/> Personal Info</h3>
                    <div className="space-y-2">
                       <p className="flex justify-between"><span className="text-gray-500">Phone:</span> <span>{selectedEmployee.phone || 'N/A'}</span></p>
                       <p className="flex justify-between"><span className="text-gray-500">Email:</span> <span>{selectedEmployee.email || 'N/A'}</span></p>
                       <p className="flex justify-between"><span className="text-gray-500">Address:</span> <span className="truncate w-32 text-right">{selectedEmployee.address || 'N/A'}</span></p>
                       <p className="flex justify-between"><span className="text-gray-500">Nationality:</span> <span>{selectedEmployee.nationality || 'N/A'}</span></p>
                    </div>
                 </div>
                 <div className="bg-blue-50 p-4 rounded border border-blue-100">
                    <h3 className="font-bold text-blue-900 mb-3 flex items-center gap-2"><CreditCard className="w-3 h-3"/> Bank Details</h3>
                    <div className="space-y-2">
                       <p className="flex justify-between"><span className="text-blue-400">Bank:</span> <span>{selectedEmployee.bankName || 'N/A'}</span></p>
                       <p className="flex justify-between"><span className="text-blue-400">Account:</span> <span className="font-mono">{selectedEmployee.bankAccountNumber || 'N/A'}</span></p>
                       <p className="flex justify-between"><span className="text-blue-400">Name:</span> <span className="truncate w-32 text-right">{selectedEmployee.bankAccountName || 'N/A'}</span></p>
                    </div>
                 </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-6">
                 <div>
                    <h4 className="font-bold text-gray-900 text-sm mb-3 flex items-center gap-2"><History className="w-4 h-4 text-gray-400" /> Career History</h4>
                    <div className="bg-gray-50 rounded-lg p-3 border border-gray-200 max-h-32 overflow-y-auto custom-scrollbar">
                      {(!selectedEmployee.statusLogs || selectedEmployee.statusLogs.length === 0) ? <p className="text-xs text-gray-400 italic">No history yet.</p> : (
                          ...
                          {selectedEmployee.statusLogs.map((log: any) => {
                            ...
                            return (
                              <div key={log.id} className="flex gap-2">
                                 ...
                                 <div><p className={clsx("text-xs font-medium leading-tight", info.color)}>{info.text}</p></div>
                              </div>
                            );
                          })}
                        </div>
                 ...
                 <div>
                    <h4 className="font-bold text-gray-900 text-sm mb-3 flex items-center gap-2"><CreditCard className="w-4 h-4 text-gray-400" /> Recent Pays</h4>
                    {employeePayrolls.length > 0 ? (
                      <div className="bg-gray-50 rounded-lg overflow-hidden border border-gray-200">
                        <table className="min-w-full text-xs">
                          <thead className="bg-gray-100"><tr><th className="px-3 py-2 text-left">Date</th><th className="px-3 py-2 text-right">Net</th></tr></thead>
                          <tbody>{employeePayrolls.slice(0, 3).map((p: any) => (<tr key={p.id} className="border-t border-gray-200"><td className="px-3 py-2 text-gray-600">{new Date(p.paymentDate).toLocaleDateString()}</td><td className="px-3 py-2 text-right font-bold text-gray-900">₦{p.netPay.toLocaleString()}</td></tr>))}</tbody>
                        </table>
                      </div>
                    ) : (<p className="text-xs text-gray-400 italic">No payment history found.</p>)}
                 </div>
              ...
              <div className="flex justify-end pt-2 border-t border-gray-100">
                 <button onClick={() => setModal(null)} className="text-sm font-medium text-gray-600 hover:text-gray-900">Close Profile</button>
              </div>
            </div>
          )
        )}
